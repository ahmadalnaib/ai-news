                  ...
                  >
                      ...
                      <span class="md:flex-grow flex flex-col items-end justify-center">
                        ...
                        <form action="{{route('admin.news.destroy',$new->slug)}}" method="post">
                          @csrf
                          @method('delete')
                          <button  onclick="return confirm('Sind Sie sicher, dass du diesen Beitrag löschen möchtest? Es gibt keinen Weg zurück.  😓')"  class="btn btn-danger" type="submit">مسح</button>
                      </form>
                      </span>
                  </div>
              @endforeach
